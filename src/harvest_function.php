...
class HarvestCycle {
    ...
    public function insertCycle($cycle_num, $start_date, $end_date, $adminID) {
        ...
        if ($add_cycle_query) {
            $this->notification->insertNotification($adminID, 'active', 'Successfully added new cycle.', 'new_cycle', 'harvest_cycle.php', 'unseen');
        } else {
            $this->notification->insertNotification($adminID, 'active', 'Failed to add new cycle.', 'failed_to_add_cycle', 'harvest_cycle.php', 'unseen');
        }
    }
    ...
    public function deleteCycle($cycle_num, $adminID) {
        ...
        if ($delete_query) {
            if (mysqli_affected_rows($this->conn) > 0) {
                $this->notification->insertNotification($adminID, 'active', 'Cycle was deleted successfully.', 'delete_cycle', 'harvest_cycle.php', 'unseen');
            } else {
                $this->notification->insertNotification($adminID, 'active', 'No cycle was deleted.', 'failed_to_delete_cycle', 'harvest_cycle.php', 'unseen');
            }
        } else {
            $this->notification->insertNotification($adminID, 'active', 'Failed to delete cycle.', 'failed_to_delete_cycle', 'harvest_cycle.php', 'unseen');
        }
    }
    ...
    public function editCycle($current_cycle_num, $new_cycle_num, $edit_start, $edit_end, $adminID) {
        ...
        $update_query = "UPDATE harvest_cycle
                         SET cycle_number = '$new_cycle_num', start_of_cycle = '$edit_start', end_of_harvest = '$edit_end'
