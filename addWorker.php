<?php
    ...
    if (isset($_POST['btn_delete'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM user_table WHERE userID = ? AND adminID = ?");
        $stmt->bind_param('ii', $user_ID, $adminID);  // Assuming userID and adminID are integers
        $stmt->execute();
    ...
    if (isset($_POST['edit_btn'])) {
        ...
        $worker_list = "SELECT userID, user_name, email, number, password FROM user_table WHERE userID = ? AND adminID = ?";
        $stmt = $conn->prepare($worker_list);
        $stmt->bind_param('ii', $user_ID, $adminID);
        $stmt->execute();
        ...
        if ($edit_name !== $current_name) {
            $stmt = $conn->prepare("UPDATE user_table SET user_name = ? WHERE userID = ?");
            $stmt->bind_param('si', $edit_name, $user_ID);
            $stmt->execute();
            $update_success = true;
        }

        // Update email and send OTP if changed
        if ($edit_email !== $current_email) {
            $otpData = $otpHandler->generateOTPUser($edit_email);

            if ($otpData) {
                $otp = $otpData['otp'];
                $otp_expiry = $otpData['otp_expiry'];

                // Update email and OTP in the database
        ...
        if ($edit_number !== $current_number) {
            $stmt = $conn->prepare("UPDATE user_table SET number = ? WHERE userID = ?");
            $stmt->bind_param('si', $edit_number, $user_ID);
            $stmt->execute();
            $update_success = true;
        }

        // Update password if changed and re-hash
        if (!password_verify($edit_password, $current_password)) {
            $hashedPassword = password_hash($edit_password, PASSWORD_BCRYPT);
            $stmt = $conn->prepare("UPDATE user_table SET password = ? WHERE userID = ?");
    ...
    if (isset($_POST['clearNotif'])) {
        $clearNotif = "DELETE FROM tblNotification WHERE adminID = ?";
    ...
    <main class="bg-light">
        <div class="p-2">
            ...
            <nav class="px-3 py-3 rounded-4">
                <div>
                    <p class="d-none d-lg-block mt-3 mx-3 fw-semibold">Welcome to Workers</p>
                </div>
